...
bootstrap da ADR 015 é o próximo passo, e antes dele é preciso conferir no
console o que veio junto — vínculo de faturamento, os seis papéis de
bootstrap e as políticas de organização. A3 venceu em 04/09 e foi atendida
...
| ~~A3~~ | ~~**Projetos GCP `dev`, `hml` e `prod`**~~ | **Atendida em 23/09**: os três projetos existem e o acesso da ness. foi liberado. Os IDs reais são `alupar-dev-alupdata`, `alupar-hm-alupdata` e `prod-alupdata` — **diferentes dos previstos** (`alupdata-dev`/`-hml`/`-prod`), e o de homologação diverge do próprio nome do projeto (`alupar-hml-alupdata`). O que resta é da ness. (N3) e o que ainda precisa ser conferido está em A13 | plano 2.2 (0.13), [ADR 015](arquitetura/decisoes/015-fundacao-do-ambiente.md), [#55](https://github.com/nessenergy/Alupdatalake/issues/55) |
| A13 | **Conferir o que veio junto com a liberação**, antes do bootstrap: conta de faturamento vinculada aos três projetos; os seis papéis da ADR 015 concedidos às contas da ness. (`serviceusage.serviceUsageAdmin`, `storage.admin`, `artifactregistry.admin`, `iam.workloadIdentityPoolAdmin`, `iam.serviceAccountAdmin`, `resourcemanager.projectIamAdmin`); `iam.allowedPolicyMemberDomains` admitindo o domínio da ness.; `gcp.resourceLocations` admitindo `us-east1` (ADR 011); `iam.workloadIdentityPoolProviders` admitindo o emissor do GitHub Actions | o `terraform apply` do bootstrap falha no meio e deixa recurso pela metade | `runbook/primeiro-deploy.md` §0, [#55](https://github.com/nessenergy/Alupdatalake/issues/55) |
...
| [#87](https://github.com/nessenergy/Alupdatalake/issues/87) | Destinatários de alerta e `billing_account` | Alup | destinatários: 11/09, atendido; billing: 18/09 | destinatários recebidos em 11/09; **o ID da conta de faturamento ainda não foi informado** — com os projetos criados em 23/09, ele pode ser lido no console assim que o acesso for verificado (A13). Sem ele o orçamento com alerta de custo não é aplicado |
