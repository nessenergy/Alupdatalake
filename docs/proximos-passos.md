...
| **1.6** | **Gravar o token do GitHub no secret `alupdata-dataform-git-token`** e apontar `DATAFORM_GIT_TOKEN_VERSAO` no ambiente `dev`, depois reexecutar o deploy | ness. (humano) | **é o que falta para o Dataform existir** — e sem Dataform não há Bronze, Silver nem Gold. O token é *fine-grained*, restrito a `nessenergy/Alupdatalake`, só *Contents: Read-only*. Passo a passo em `runbook/primeiro-deploy.md` §3 |
