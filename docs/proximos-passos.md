...
| **1.5** | **Primeiro `terraform apply` do `infra/` em `dev`**, pelo workflow `Deploy GCP`, módulo `all` | ness. | cria datasets, bucket raw, secrets vazios, Cloud Run e Scheduler. Pré-condições e conferências em `runbook/primeiro-deploy.md` §1–§3. O `billing_account` fica vazio até o valor do orçamento ser acertado (item 2.13) |
